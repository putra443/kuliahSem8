...
class BST <T extends Comparable<T>>{
    ...
    private String preorder(Node x){             //membuat tree yang akan di print dengan susunan preorder
        if(x==null)
            return "";                         //jika node bernilai null, maka return ;
        if(x.left == null && x.right == null)   //jika anak kiri dan kanan node bernilai null maka return node;
            return x.toString();
        else
            return x.toString() + preorder(x.left) + preorder(x.right); //jika memiliki anak kanan / anak kiri return dengan urutan anak kiri , node x , anak kanan
    }
    private String postorder(Node x){             //membuat tree yang akan di print dengan susunan postorder
        if(x==null)
            return "";                         //jika node bernilai null, maka return ;
        if(x.left == null && x.right == null)   //jika anak kiri dan kanan node bernilai null maka return node;
            return x.toString();
        else
            return postorder(x.left) + postorder(x.right) + x.toString(); //jika memiliki anak kanan / anak kiri return dengan urutan anak kiri , node x , anak kanan
    }
    ...
    public T successor(T curr){
        Node result = search(curr, root);
        if(result==null){
            return null;
        }
        else{
            Node akhir = successor(result);
            if(akhir ==null){
                return null;
            }
            else{
    ...
    public T predecessor(T curr){
        Node result = search(curr, root);
        if(result==null){
            return null;
        }
        else{
            Node akhir = predecessor(result);
            if(akhir ==null){
                return null;
            }
            else{
    ...
    public static void main (String[]args){
        ...
        while(!input.equals("stop")){
            if(input.equals("insert")){
                ...
                if(bst.search(angka)==null){
                    bst.insert(angka);
                    output[i]= input + " " +angka + " " + "berhasil";
                }
                else{
                    output[i]= input + " " +angka + " " + "gagal, data sudah ada di pohon";
                }
                ...
                int angka = sc.nextInt();
                if(bst.search(angka)!=null){
                    output[i]= input + " " + "data" + " " + angka + ": ditemukan";
                }
                else{
                    output[i]= input + " " + "data" + " " + angka + ": tidak ditemukan";
                }
            }
            else if(input.equals("print")){
                output[i] = "Pohon saat ini: " + bst.toString();
            }
            else if(input.equals("successor")){
                int angka = sc.nextInt();
                if(bst.successor(angka)!=null){
                    output[i] = "successor dari " + angka + " adalah " + bst.successor(angka);
                }
                else{
                    output[i] = "successor dari " + angka + " adalah tidak ada"; 
                }
            }
            else if(input.equals("predecessor")){
                int angka = sc.nextInt();
                if(bst.predecessor(angka)!=null){
                    output[i] = "predecessor dari " + angka + " adalah " + bst.predecessor(angka);
                }
                else{
                    output[i] = "predecessor dari " + angka + " adalah tidak ada"; 
                }
            }
            else if(input.equals("delete")){
                int angka = sc.nextInt();
                if(bst.delete(angka)==true){
                    output[i] = "hasil delete " + angka + ": true";
                }
                else{
                    output[i] = "hasil delete " + "-1" + ": false";
                }
            }
            else if(input.equals("max")){
                output[i] = "maximum pada pohon: "+bst.max();
            }
            else if(input.equals("min")){
                output[i] = "minimum pada pohon: "+bst.min();
            }
            i++;
            input = sc.next();
        }
